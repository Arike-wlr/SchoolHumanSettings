...
function _saveCache(key, data) { try { sessionStorage.setItem(key, JSON.stringify(data)); } catch(e) {} }
...
VM.index = (function() {
  ...
  function renderImagePreview() {
    ...
    grid.innerHTML = items.map(function(it) {
      return '<div style="position:relative;aspect-ratio:1;border-radius:6px;overflow:hidden;border:1px solid var(--border);">' +
        '<img src="'+it.url+'" alt="预览" style="width:100%;height:100%;object-fit:cover;">' +
  ...
  function renderDetailImage() {
    ...
    var thumbs=imgs.length>1?imgs.map(function(u,i){return '<img src="'+u+'" onclick="VM.index.detailImageGo('+i+')" style="width:42px;height:42px;object-fit:cover;border-radius:4px;cursor:pointer;border:2px solid '+(i===detailImageIdx?'var(--primary)':'transparent')+';">';}).join(''):'';
    wrap.innerHTML='<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">'+prev+'<div style="flex:1;text-align:center;min-width:0;"><img src="'+imgs[detailImageIdx]+'" style="max-width:100%;max-height:240px;border-radius:6px;object-fit:contain;">'+cnt+'</div>'+next+'</div>'+(thumbs?'<div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">'+thumbs+'</div>':'');
  ...
  function cardHTML(c, gIdx) {
    ...
    if(cu&&cu.trim()!==''){
      ...
      ph='<div style="flex-shrink:0;width:88px;position:relative;align-self:stretch;margin-right:4px;margin-top:4px;min-height:64px;"><div style="width:88px;height:100%;border-radius:4px;overflow:hidden;border:1px solid var(--border);background:#f8f9fa;"><img src="'+cu+'" alt="'+esc(c.name)+'" loading="lazy" style="width:100%;height:100%;object-fit:cover;"></div>'+bd+'</div>';
    }
    var tf='';
      ...
      if (Array.isArray(content)) {
        for (var i = 0; i < content.length; i++) {
          ...
          if (isNewFormat && item.type === 'image') {
            ...
            html += '<div class="doc-image"><img src="' + imgUrl + '" alt="' + esc(item.caption || '') + '" loading="lazy" onerror="this.style.display=\'none\'">' + (item.caption ? '<div class="doc-image-caption">' + esc(item.caption) + '</div>' : '') + '</div>';
