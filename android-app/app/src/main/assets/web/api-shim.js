...
async function handleApiRequest(url, init) {
  ...
  try {
    ...
    // ---- 全局搜索（离线：从 IndexedDB 本地检索；接口与桌面版一致）----
    if (path === '/api/search' && method === 'GET') {
      ...
      for (const c of chars) {
        ...
        if (matched_field) {
          characters.push({
            id: c.id, name: c.name, university: c.university || '', region: c.region || '',
      ...
      const worldview = worlds.filter(w => hit(w.title) || hit(w.category) || hit(w.main_category) || hit(w.content))
        .slice(0, MAX)
        .map(w => ({
          id: w.id, title: w.title, category: w.category || '', main_category: w.main_category || '',
